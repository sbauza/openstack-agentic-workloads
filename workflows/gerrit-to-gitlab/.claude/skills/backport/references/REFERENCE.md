# Backport — Detailed Process Reference

This document contains the full step-by-step process for the backport skill. It is loaded on demand when the `/backport` skill is activated.
...
## Step 4: Analyze Dependencies and Check for Reverts
...
## Step 5: Prompt for Target

**Check for an existing backport branch first.** Look for a local branch matching the pattern `backport/*-to-<stable_branch_pattern>`:
...
If the cherry-pick succeeds cleanly, proceed to step 8 (Augment Commit Message).

If the cherry-pick fails due to conflicts, proceed to the Conflict Handling section.
...
6. **After user confirms resolution**:
   ...
   - **Collect conflict resolution notes** for the commit message. For each conflicting file, ask the user:
     ...
     Store the list of `(file_path, resolution_description)` pairs for use in the commit message augmentation step.
...
After successful conflict resolution and cherry-pick continuation, proceed to commit message augmentation.

## Step 8: Augment Commit Message
...
- `Conflicts:` is only added if the cherry-pick had merge conflicts. Each entry lists the conflicting file path and the user's description of how it was resolved. The description may reference a Change-Id if the conflict was caused by a missing prerequisite change.
...
Ask: "Commit added. You can run `/backport` to add more commits, or run `/create-mr` to create the GitLab merge request with all {N} commits."
